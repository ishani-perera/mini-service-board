...
export default function JobCard({ job }) {
  ...
  return (
    <Link href={`/jobs/${job._id}`}>
      <div className="bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 rounded-3xl border border-amber-100 hover:border-amber-200 hover:shadow-2xl hover:-translate-y-2 active:scale-95 transition-all duration-400 cursor-pointer h-full flex flex-col overflow-hidden group">
        
        {/* Header with category and badge */}
        <div className="px-6 sm:px-8 pt-6 sm:pt-8 pb-4 border-b border-amber-100/50">
          <div className="flex items-center justify-between mb-3">
            <span className="inline-block bg-amber-100 text-amber-900 text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl">
        ...
        <div className="p-6 sm:p-8 flex-1 flex flex-col gap-5">
          <div className="space-y-3">
            <h3 className="font-black text-slate-900 text-lg sm:text-xl leading-tight group-hover:text-amber-700 transition-colors duration-300 line-clamp-2">
              {job.title}
            </h3>
          ...
          {/* Footer */}
          <div className="mt-auto flex items-center justify-between pt-6 border-t border-amber-100/50">
            <div className="flex flex-col">
               <span className="text-[10px] font-bold text-amber-700 uppercase tracking-wider mb-1">Budget</span>
               <span className="text-lg sm:text-xl font-black text-amber-800 group-hover:text-amber-900 transition-colors">
